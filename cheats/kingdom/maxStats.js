...
(() => {
    ...
    let img = new Image;
    img.src = "https://raw.githubusercontent.com/005Konz/Blooket-Cheats/main/autoupdate/timestamps/kingdom/maxStats.png?" + Date.now();
    ...
    img.onload = function() {
        ...
        let _, time = 1716689128021, error = "There was an error checking for script updates. Run cheat anyway?";
        try {
            [_, time, error] = decode.match(/LastUpdated: (.+?); ErrorMessage: "((.|\n)+?)"/);
        } catch (e) {}
        if (parseInt(time) <= 1716689128021 || iframe.contentWindow.confirm(error)) cheat();
    }
