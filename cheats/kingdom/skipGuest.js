...
(() => {
    ...
    let img = new Image;
    img.src = "https://raw.githubusercontent.com/005Konz/Blooket-Cheats/main/autoupdate/timestamps/kingdom/skipGuest.png?" + Date.now();
    ...
    img.onload = function() {
        ...
        let _, time = 1716689128114, error = "There was an error checking for script updates. Run cheat anyway?";
        try {
            [_, time, error] = decode.match(/LastUpdated: (.+?); ErrorMessage: "((.|\n)+?)"/);
        } catch (e) {}
        if (parseInt(time) <= 1716689128114 || iframe.contentWindow.confirm(error)) cheat();
    }
