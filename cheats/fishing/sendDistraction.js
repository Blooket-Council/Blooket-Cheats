...
(() => {
    ...
    img.onload = function() {
        ...
        let _, time = 1716689126112, error = "There was an error checking for script updates. Run cheat anyway?";
        try {
            [_, time, error] = decode.match(/LastUpdated: (.+?); ErrorMessage: "((.|\n)+?)"/);
        } catch (e) {}
        if (parseInt(time) <= 1716689126112 || iframe.contentWindow.confirm(error)) cheat();
    }
