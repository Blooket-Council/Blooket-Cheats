...
(() => {
    ...
    img.onload = function() {
        ...
        let _, time = 1730670518748, error = "There was an error checking for script updates. Run cheat anyway?";
        try {
            [_, time, error] = decode.match(/LastUpdated: (.+?); ErrorMessage: "((.|\n)+?)"/);
        } catch (e) {}
        if (parseInt(time) <= 1730670518748 || iframe.contentWindow.confirm(error)) cheat();
    }
