...
(() => {
    ...
    let img = new Image;
    img.src = "https://raw.githubusercontent.com/05Konz/Blooket-Cheats/main/autoupdate/timestamps/brawl/magnet.png?" + Date.now();
    ...
    img.onload = function() {
        ...
        let _, time = 1710881111969, error = "There was an error checking for script updates. Run cheat anyway?";
        try {
            [_, time, error] = decode.match(/LastUpdated: (.+?); ErrorMessage: "((.|\n)+?)"/);
        } catch (e) {}
        if (parseInt(time) <= 1710881111969 || iframe.contentWindow.confirm(error)) cheat();
    }
